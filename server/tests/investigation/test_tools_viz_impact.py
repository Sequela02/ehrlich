...
class TestRenderProgramDashboard:
    ...
    async def test_basic(self) -> None:
        result = json.loads(await render_program_dashboard(
            indicators=[
                {"name": "Enrollment", "baseline": 70.0, "target": 90.0,
                 "actual": 85.0, "unit": "%"},
                {"name": "Attendance", "baseline": 60.0, "target": 80.0,
                 "actual": 35.0, "unit": "%"},
            ],
...
class TestRenderGeographicComparison:
    @pytest.mark.anyio
    async def test_basic(self) -> None:
        result = json.loads(await render_geographic_comparison(
            regions=[
...
class TestRenderParallelTrends:
    @pytest.mark.anyio
    async def test_basic(self) -> None:
        result = json.loads(await render_parallel_trends(
            treatment_series=[
