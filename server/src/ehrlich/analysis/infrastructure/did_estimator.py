...
from ehrlich.analysis.domain.causal_ports import DiDEstimatorPort
...
class DiDEstimator(DiDEstimatorPort):
    ...
    def estimate(
        ...
        se_treatment = math.sqrt(
            var_t_pre / max(len(treatment_pre), 1) + var_t_post / max(len(treatment_post), 1)
        )
        se_control = math.sqrt(
            var_c_pre / max(len(control_pre), 1) + var_c_post / max(len(control_post), 1)
        )
        ...
        if min_n < 5:
            threats.append(
                ThreatToValidity(
                    ...
                    severity="high",
                    description=(
                        f"Minimum group size is {min_n}. Small samples reduce statistical power."
                    ),
        ...
        if 0.01 < p_value < 0.05:
            threats.append(
                ThreatToValidity(
                    ...
                    severity="low",
                    description=(
                        f"Result is statistically significant but borderline (p={p_value:.4f})."
                    ),
