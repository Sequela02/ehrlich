      ...
      {stored ? (
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="sk-ant-..."
            className="flex-1 rounded border border-border bg-background px-3 py-1.5 font-mono text-xs placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-primary/40"
          />
          <button
            onClick={handleSave}
            disabled={!value.trim()}
            className="rounded bg-primary px-3 py-1.5 font-mono text-[11px] font-medium text-primary-foreground transition-all hover:brightness-110 disabled:opacity-50"
          >
            Save
          </button>
        </div>
